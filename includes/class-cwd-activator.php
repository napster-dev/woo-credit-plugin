...
class CWD_Activator {
	...
	public static function activate() {
		...
		$product_id = get_option( 'cwd_credit_payment_product_id' );
		...
		if ( $product_id && 'publish' === get_post_status( $product_id ) ) {
			...
			if ( ! is_wp_error( $new_product_id ) ) {
				...
				update_option( 'cwd_credit_payment_product_id', $new_product_id );
			}
		}

		// Add rewrite rules and flush
		// We will call the init function of endpoints to register them before flushing
		if ( class_exists( 'CWD_Endpoints' ) ) {
			CWD_Endpoints::add_endpoints();
		}
